...
def parse_post_title_and_date(typ_file: Path) -> tuple[str | None, datetime | None]:
    """
    从文章 .typ 文件中提取 title 和 date 元数据。
    """
    try:
        content = typ_file.read_text(encoding="utf-8")
    except Exception:
    ...
    date_match = re.search(
        r"date\s*:\s*datetime\(\s*year\s*:\s*(\d{4})\s*,\s*month\s*:\s*(\d{1,2})\s*,\s*day\s*:\s*(\d{1,2})\s*\)",
        content,
        re.DOTALL,
    )
    ...
    try:
        ...
        return title, datetime(y, m, d)
    except ValueError:
        return title, None


def update_home_recent_posts(limit: int = 5) -> bool:
    """
    自动更新首页“最近更新”列表（按 date 元数据倒序）。

    仅更新 content/index.typ 中 AUTO_RECENT_START/END 标记之间的内容。
    """
    home_typ = CONTENT_DIR / "index.typ"
    if not home_typ.exists():
        return True

    start_marker = "// AUTO_RECENT_START"
    end_marker = "// AUTO_RECENT_END"

    try:
        home_content = home_typ.read_text(encoding="utf-8")
    except Exception as e:
        print(f"❌ 读取首页文件失败: {e}")
        return False

    if start_marker not in home_content or end_marker not in home_content:
        print("⚠️ 首页缺少自动更新标记，跳过最近更新列表刷新。")
        return True

    posts: list[tuple[datetime, str, str]] = []

    for typ_file in CONTENT_DIR.rglob("index.typ"):
        if typ_file == home_typ:
            continue
        ...
        # 仅纳入有 date 元数据的文章页
        title, date = parse_post_title_and_date(typ_file)
        if not title or not date:
            continue

        post_dir = rel_path.parent.as_posix()
        if post_dir == ".":
            continue

        posts.append((date, post_dir, title))

    posts.sort(key=lambda item: (item[0], item[1]), reverse=True)

    lines: list[str] = []
    for date, post_dir, title in posts[:limit]:
        safe_title = title.replace("]", "\\]")
        lines.append(f'- `{date.strftime("%Y-%m-%d")}` #link("/{post_dir}/")[{safe_title}]')
    ...
    updated_content = pattern.sub(new_block, home_content, count=1)

    if updated_content == home_content:
        return True

    try:
        home_typ.write_text(updated_content, encoding="utf-8")
        print(f"✅ 首页最近更新已刷新: {len(lines)} 条")
        return True
    except Exception as e:
        print(f"❌ 写入首页文件失败: {e}")
        return False
...
def get_website_url_from_config() -> str:
    """
    从 config.typ 读取 website-url，读取失败时返回默认值。
    """
    default_url = "https://blog17.github.io/"
    if not CONFIG_FILE.exists():
        return default_url

    try:
        content = CONFIG_FILE.read_text(encoding="utf-8")
        match = re.search(r'website-url\s*:\s*"([^"]+)"', content)
...
def build_year_bars(posts: list[tuple[datetime, str, str]], max_width: int = 10) -> list[str]:
    """
    按年份生成简单文本柱状图（右侧栏展示）。
    """
    year_counts: dict[int, int] = {}
    for date, _, _ in posts:
        year_counts[date.year] = year_counts.get(date.year, 0) + 1
...
def update_home_sidebar() -> bool:
    ...
    start_marker = "// AUTO_SIDEBAR_START"
    end_marker = "// AUTO_SIDEBAR_END"
    ...
    posts.sort(key=lambda item: (item[0], item[1]), reverse=True)
    latest_date = posts[0][0].strftime("%Y-%m-%d") if posts else "N/A"

    categories = sorted(
        {
            p.relative_to(CONTENT_DIR).parts[0]
            for p in CONTENT_DIR.rglob("index.typ")
            if p != home_typ and len(p.relative_to(CONTENT_DIR).parts) >= 3
        }
    )
    ...
    encoded_text = urllib.parse.quote("博十七's Blog", safe="")

    year_bar_lines = build_year_bars(posts)

    lines = [
        "#tufted.margin-note[",
        '#html.span(class: "home-sidebar", [',
        "*分享* #linebreak()",
        f'#link("https://twitter.com/intent/tweet?url={encoded_url}&text={encoded_text}")[X / Twitter] · '
        f'#link("https://www.facebook.com/sharer/sharer.php?u={encoded_url}")[Facebook] · '
        f'#link("mailto:?subject={encoded_text}&body={encoded_url}")[Email]',
        "#linebreak() #linebreak()",
        "*更新统计* #linebreak()",
        f"总文章：{total_posts} #linebreak()",
        f"最近更新：{latest_date} #linebreak()",
        f"栏目：{category_text} #linebreak()",
        "#linebreak()",
    ]

    for i, yline in enumerate(year_bar_lines):
        suffix = " #linebreak()" if i < len(year_bar_lines) - 1 else ""
        lines.append(f"{yline.removeprefix('- ')}{suffix}")
...
def build_html(force: bool = False) -> bool:
    ...
    if not update_home_recent_posts():
        return False
    if not update_home_sidebar():
        return False
...
def ensure_site_index() -> bool:
    """
    确保 _site/index.html 存在。

    当项目未提供 content/index.typ 时，自动生成一个简单入口页，
    避免本地预览根路径返回 404。
    """
    ...
    # 优先收集顶级目录的 index.html（如 /人物/、/时事/）
    for item in sorted(SITE_DIR.iterdir()):
        if item.is_dir() and (item / "index.html").exists():
            href = f"{item.name}/"
            links.append((href, item.name))

    # 再补充顶级 html 文件（排除 index.html）
    for item in sorted(SITE_DIR.glob("*.html")):
        if item.name != "index.html":
            links.append((item.name, item.stem))

    if not links:
        return True

    first_href, first_label = links[0]
    list_items = "\n".join(
        f'      <li><a href="{html.escape(href)}">{html.escape(label)}</a></li>'
        for href, label in links
    )
    redirect_target = html.escape(first_href)
...
<html lang="zh-CN">
  ...
  <body>
    <main>
      <h1>正在跳转...</h1>
      <p>如果没有自动跳转，请点击：<a href="/{redirect_target}">{html.escape(first_label)}</a></p>
      <ul>
{list_items}
      </ul>
    </main>
            ...
            if index_path.exists():
                ...
                first_html = next(
                    (
                        ...
                        if p.relative_to(SITE_DIR).as_posix() != "index.html"
                    ),
                    "",
                )
                open_path = f"/{first_html}" if first_html else ""
...
def get_site_url() -> str | None:
    ...
    if not index_html.exists():
        print(f"⚠️ 跳过站点 URL 解析: 未找到 {index_html}")
        return None
...
def build(force: bool = False) -> bool:
    ...
    results.append(ensure_site_index())
